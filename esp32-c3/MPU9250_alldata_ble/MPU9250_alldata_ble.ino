/*
  ESP32-C3 MPU9250 BLE Server

  This program reads accelerometer and gyroscope data from an MPU9250
  and sends it over Bluetooth Low Energy (BLE) using a notify characteristic.

  It emulates a common "UART-like" service.
*/

// Core MPU9250 libraries
...
void loop() {
  ...
  if (deviceConnected) {
    xyzFloat gValue = myMPU9250.getGValues(); // Accel data
    xyzFloat gyr = myMPU9250.getGyrValues();   // Gyro data
    xyzFloat magValue = myMPU9250.getMagValues(); // Mag data

    // Format the data into a single string
    // Format: "ax,ay,az,gx,gy,gz"
    // Example: "0.01,-0.02,0.99,1.23,-0.45,0.05"
    char dataString[80]; // Buffer to hold the data
    snprintf(dataString, sizeof(dataString), "%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f", 
